...
/*   Updated: 2023/09/29 20:58:56 by acosi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/*
	Sends a "game over" signal when the player moves to the position
	of a patrol ('X' = regular patrol marker ; 'Y' = patrol that has
	just been moved).
*/
...
int	game_over(int keycode, t_context *context, int x, int y)
{
	if ((keycode == 122 || keycode == 119)
		...
		|| context->map[y - 1][x] == 'Y'))
		return (1);
	if (keycode == 115 && (context->map[y + 1][x] == 'X'
		|| context->map[y + 1][x] == 'Y'))
		return (1);
	if ((keycode == 113 || keycode == 97) && (context->map[y][x - 1] == 'X'
		|| context->map[y][x - 1] == 'Y'))
		return (1);
	if (keycode == 100 && (context->map[y][x + 1] == 'X'
		|| context->map[y][x + 1] == 'Y'))
		return (1);
	return (0);
}
