...
/*   Updated: 2023/09/30 23:58:32 by acosi            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
//	Put a random border sprite to the main window to make the layout
//	of the asteroids less repetitive.
...
void	put_border(t_context *context, int x, int y)
{
	...
	if (r == 0)
		mlx_put_image_to_window(context->game, context->window,
			context->data[0].img_ptr, x * 50, y * 50);
	if (r == 1)
		mlx_put_image_to_window(context->game, context->window,
			context->data[9].img_ptr, x * 50, y * 50);
	if (r == 2)
		mlx_put_image_to_window(context->game, context->window,
			context->data[10].img_ptr, x * 50, y * 50);
	if (r == 3)
		mlx_put_image_to_window(context->game, context->window,
			context->data[11].img_ptr, x * 50, y * 50);
}
